...
def main():
    ...
    if distil_args.do_distil:
        ...
        server_handler.load_distilled_student()

    # set trainable for lora
    logger.info("====Finished distillation===")
    server_handler.set_plugin_params()
    ...
    # build model for clients, deleting teacher module
    # client_model = server_handler.send_client_model()
    client_model = AutoModelForCausalLM.from_pretrained(model_args.model_name_or_path, config=config,
                                                        trust_remote_code=True)
    # 检查新模型的结构
    print(client_model)

    fed_trainer = FedTrainer(client_model, model_args, training_args, data_args, tokenizer, client_datasets,
                             len(client_datasets))
    client_ids = [i for i in range(len(client_datasets))]
    aggregator = Aggregators()
    for rd in range(fed_args.rounds):
        cur_params = serialize_model_trainable(client_model)
        ...
        deserialize_model_trainable(client_model, agg_params)
        deserialize_model_trainable(server_handler.model.teacher, agg_params)  # plugin

    # save the latest trainable model params.
    to_teacher(server_handler.model, distil_args.student_l_pad, distil_args.student_r_pad)
    server_handler.model.save_pretrained(os.path.join(training_args.output_dir, 'server_LLM'))
